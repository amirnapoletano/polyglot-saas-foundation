	...
	<header class="border-b border-border bg-surface">
		<div class="mx-auto max-w-6xl flex items-center justify-between px-6 py-4">
			<div class="flex items-center gap-4">
				...
				<span class="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">Super Admin</span>
			</div>
			<div class="flex items-center gap-4">
				<span class="text-sm text-text-secondary">{data.adminUser?.email}</span>
				<a href="/app/dashboard" class="text-sm text-brand-600 hover:text-brand-700 font-medium">Back to App</a>
			</div>
		</div>
