...
export const actions: Actions = {
  createWorkspace: async ({ request, locals }) => {
    ...
    if (!name || name.length < 2) return fail(400, { message: 'Workspace name must be at least 2 characters.' });
    if (name.length > 60) return fail(400, { message: 'Workspace name must be under 60 characters.' });
    ...
    if (orgError || !org) return fail(500, { message: orgError?.message ?? 'Failed to create workspace.' });
