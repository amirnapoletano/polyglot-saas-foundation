...
{#if !data.isPro}
	...
	<Card>
		<div class="text-center py-8">
			<svg class="mx-auto h-12 w-12 text-text-tertiary mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
	...
	{#if newKey}
		<Card class="border-brand-200 dark:border-brand-800 mb-6">
			<div class="flex items-start gap-3">
				<svg class="h-5 w-5 text-brand-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z" />
				</svg>
				<div class="flex-1 min-w-0">
					<p class="text-sm font-medium text-text-primary">Your new API key — copy it now, it won't be shown again.</p>
					<div class="mt-2 flex items-center gap-2">
						<code class="flex-1 rounded-lg bg-surface-tertiary px-3 py-2 text-sm font-mono text-text-primary break-all">{newKey}</code>
	...
	<Card>
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="mt-1 text-sm text-text-secondary">Keys authenticate requests to <code class="text-xs bg-surface-tertiary rounded px-1 py-0.5">/api/v1/*</code></p>
			</div>
					...
					<div class="flex items-center justify-between py-3">
						<div class="min-w-0 flex-1">
							...
							<div class="flex items-center gap-3 mt-0.5">
								...
								<span class="text-xs text-text-tertiary">Last used: {timeAgo(key.last_used_at)}</span>
								{#if key.expires_at}
									<span class="text-xs text-amber-600">Expires {new Date(key.expires_at).toLocaleDateString()}</span>
								{/if}
							</div>
						</div>
						{#if data.currentUserRole === 'owner' || data.currentUserRole === 'admin'}
							<Button variant="danger" size="sm" onclick={() => { revokeKeyId = key.id; revokeOpen = true; }}>Revoke</Button>
						{/if}
					</div>
				{/each}
			</div>
...
{#if createOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
		...
		<div class="relative w-full max-w-sm rounded-xl border border-border bg-surface p-6 shadow-xl">
			...
			<div class="mt-4">
				<Input label="Key name" bind:value={keyName} placeholder="e.g. Production Backend" maxlength={100} />
			</div>
...
<ConfirmModal
	...
	onconfirm={revokeKey}
	oncancel={() => { revokeOpen = false; revokeKeyId = null; }}
/>
