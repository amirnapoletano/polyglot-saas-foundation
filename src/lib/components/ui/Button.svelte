<script lang="ts">
	...
	const variants: Record<string, string> = {
		...
		secondary: 'bg-surface border border-border text-text-primary hover:bg-surface-secondary focus:ring-brand-500',
		danger: 'bg-red-600 text-white hover:bg-red-700 focus:ring-red-500',
		ghost: 'text-text-secondary hover:text-text-primary hover:bg-surface-secondary focus:ring-brand-500'
	};
...
<button class="{base} {variants[variant]} {sizes[size]} {className}" disabled={loading || rest.disabled} {...rest}>
	{#if loading}
